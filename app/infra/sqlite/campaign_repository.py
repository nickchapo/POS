...
class CampaignRepository:
    ...
    def _campaign_to_row(self, campaign: Campaign) -> Dict[str, Any]:
        ...
        if isinstance(campaign, DiscountCampaign):
            ...
            parameters = {
                "combo_products": [str(p) for p in campaign.combo_products]
                if campaign.combo_products
                else [],
        ...
        if campaign_type == CampaignType.DISCOUNT:
            return DiscountCampaign(
                **base_args,
                ...
                product_id=UUID(parameters.get("product_id"))
                if parameters.get("product_id")
                else None,
            ...
            return ComboCampaign(
                **base_args,
                combo_products=[UUID(p) for p in parameters.get("combo_products", [])]
                if parameters.get("combo_products")
                else [],
    ...
    def save(self, campaign: Campaign) -> Campaign:
        ...
        if campaign.id is None:
            ...
            cursor.execute(
                f"INSERT INTO campaigns ({columns}) VALUES ({placeholders})", values
            )
    ...
    def find_active_for_product(self, product_id: UUID) -> List[Campaign]:
        ...
        for campaign in all_active:
            if (
                isinstance(campaign, DiscountCampaign)
                and campaign.product_id == product_id
            ):
                result.append(campaign)
            elif (
                isinstance(campaign, BuyNGetNCampaign)
                and campaign.product_id == product_id
            ):
                result.append(campaign)
            elif (
                isinstance(campaign, ComboCampaign)
                and product_id in campaign.combo_products
            ):
                result.append(campaign)
    ...
    def find_active_receipt_campaigns(self) -> List[Campaign]:
        ...
        return [
            c
            for c in all_active
            if isinstance(c, DiscountCampaign) and c.min_total is not None
        ]
